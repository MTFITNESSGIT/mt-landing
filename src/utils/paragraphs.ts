export const paragraph = [
  {
    title: "PLAN ENTRENAMIENTO PERSONALIZADO",
    texts: [
      "Te ofrezco la oportunidad de tener una rutina de entrenamiento completamente personalizada, diseñada específicamente para ti y adaptada a tus objetivos y al número de días que puedas dedicar al ejercicio. Ya sea que busques perder peso, ganar masa muscular o simplemente mantenerte en forma, crearé un plan que se ajuste a tus necesidades y estilo de vida.",
      "Cada ejercicio de tu rutina incluirá un video explicativo, lo que te permitirá aprender la técnica correcta y mejorar progresivamente tanto en forma como en peso. No solo te guiaré a través de los movimientos, sino que también te ayudaré a entender la lógica detrás de cada ejercicio. Esto significa que dejarás de depender del profesor del gimnasio que presta poca atención a tus necesidades y tendrás la confianza para entrenar de manera efectiva.",
      "Además, al enfocarte en la técnica adecuada, reducirás el riesgo de lesiones, lo que es fundamental para un progreso sostenido y saludable. Estoy aquí para acompañarte en cada paso del camino, asegurándome de que no solo cumplas tus metas, sino que también disfrutes del proceso.",
    ],
    switch: false,
  },
  {
    title: "PLAN ALIMENTACION PERSONALIZADO",
    texts: [
      "Te ofrezco la oportunidad de tener un plan de alimentación totalmente personalizado, diseñado específicamente para ti y adaptado a tus objetivos nutricionales. Ya sea que busques perder peso, ganar masa muscular o simplemente llevar un estilo de vida más saludable, crearé un programa que se ajuste a tus necesidades y preferencias alimenticias.",
      "Cada comida y snack estará cuidadosamente planeado para asegurarte de que obtienes todos los nutrientes necesarios y que disfrutas de lo que comes. Te proporcionaré recetas y recomendaciones que se adapten a tus gustos, para que no solo sigas el plan, sino que también disfrutes del proceso.",
      "Con este enfoque, dejarás de sentirte perdido en cuanto a qué comer y podrás tomar decisiones informadas que te acerquen a tus metas. Te acompañaré en cada paso, asegurándome de que tengas el apoyo y la orientación que necesitas para hacer de la alimentación saludable un hábito sostenible y agradable.",
    ],
    switch: true,
  },
  {
    title: "EBOOK DE REGALO",
    texts: [
      "Imagina tener a tu disposición una estrategia efectiva y basada en ciencia para perder grasa de manera definitiva, sin dietas extremas ni entrenamientos inalcanzables. En este ebook, Tomás Medina te ofrece un plan claro y directo, diseñado para adaptarse a tu estilo de vida y objetivos personales.",
      "Aprenderás a comprender cómo funciona realmente tu cuerpo, a comer de forma inteligente sin sacrificar el placer de la comida, y a ejercitarte de manera eficiente, optimizando cada sesión para obtener los mejores resultados. Con una combinación de hábitos saludables y rutinas efectivas, este libro te permitirá no solo quemar grasa, sino también mantener tus logros a largo plazo.",
      "Cada capítulo te guiará con información detallada y consejos prácticos, apoyados por la experiencia y el conocimiento de Tomás. Tendrás el control completo de tu proceso, entendiendo no solo qué hacer, sino también por qué hacerlo.",
    ],
    switch: false,
  },
];
